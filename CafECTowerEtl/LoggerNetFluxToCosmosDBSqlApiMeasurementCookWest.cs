...
{
    public static class LoggerNetFluxToCosmosDBSqlApiMeasurementCookWest
    {
        [FunctionName("LoggerNetFluxToCosmosDBSqlApiMeasurementCookWest")]
        public static async Task Run(
            [BlobTrigger("ectower-cookwest/raw/Flux/{name}", Connection = "ltarcafdatastreamConnectionString")]Stream myBlob, 
            string name, 
            TraceWriter log,
            ExecutionContext context)
        {
            ...
            EtlEvent etlEvent = new EtlEvent(
                ...
                "1.1", "LoggerNetFluxToCosmosDBSqlApiMeasurementCookWest",
                DateTime.UtcNow);
            etlEvent.Outputs = null;
            etlEvent.Inputs.Add($"ectower-cookwest/raw/Flux/{name}");
